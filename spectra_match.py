...
# ---------------------------------------------------------
txt_file_path = input("Enter path to unknown .txt file: ").strip('\'"')

# Ask the user where to save everything (file-explorer popup).
base_dir = choose_output_dir(DEFAULT_OUTPUT_DIR)
...
try:
    os.startfile(report_path)   # Windows: opens it in the default browser
except Exception:
    pass